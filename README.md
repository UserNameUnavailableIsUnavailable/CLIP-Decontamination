...
# If you see errors like "torch._dynamo ... has no attribute 'disable'",
# ensure torch/torchvision are version-matched (SimFeatUp installs may upgrade them).
# For CUDA 11.8:
#   pip install --force-reinstall torch==2.1.2+cu118 torchvision==0.16.2+cu118 --index-url https://download.pytorch.org/whl/cu118
# For CPU-only:
#   pip install --force-reinstall torch==2.1.2 torchvision==0.16.2 --index-url https://download.pytorch.org/whl/cpu

# If model downloads fail due to SSL inspection (self-signed certs), set one of:
#   OPEN_CLIP_CA_BUNDLE=/path/to/your/ca-bundle.pem
#   OPEN_CLIP_SSL_NO_VERIFY=1   (last resort; disables SSL verification)
```
