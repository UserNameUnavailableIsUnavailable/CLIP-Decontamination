...
def _get_ssl_context():
    ca_bundle = os.getenv("OPEN_CLIP_CA_BUNDLE") or os.getenv("SSL_CERT_FILE")
    no_verify = os.getenv("OPEN_CLIP_SSL_NO_VERIFY", "").lower() in {"1", "true", "yes"}

    if no_verify:
        warnings.warn("OPEN_CLIP_SSL_NO_VERIFY is set; SSL verification is disabled for model downloads")
...
def download_pretrained_from_url(
    ...
    if ssl_context is None:
        ...
        source = urllib.request.urlopen(url, context=ssl_context)

    with source, open(download_target, "wb") as output:
