...
def iSAID_convert_from_color(arr_3d, palette=iSAID_invert_palette, allow_bgr_fallback=True):
    ...
    if allow_bgr_fallback and arr_2d.max() == 0 and np.any(arr_3d != 0):
        arr_2d_bgr = iSAID_convert_from_color(arr_3d[..., ::-1], palette, False)
        if arr_2d_bgr.max() > 0:
            return arr_2d_bgr
...
def main():
    ...
    if not available_splits:
        raise FileNotFoundError(f'No split folders found in {dataset_path}')

    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        for dataset_mode in available_splits:

            # for dataset_mode in [ 'test']:
            print(f'Extracting  {dataset_mode}')
