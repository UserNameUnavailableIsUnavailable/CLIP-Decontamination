...
def slide_crop_image(src_path, out_dir, mode, patch_H, patch_W, overlap):
    ...
    for x in range(0, img_W, patch_W - overlap):
        for y in range(0, img_H, patch_H - overlap):
            ...
            save_path_image = osp.join(out_dir, 'images', mode, str(image))
    ...
    for x in range(0, img_W, patch_W - overlap):
        for y in range(0, img_H, patch_H - overlap):
            ...
            image = pre_name + osp.basename(src_path).split('.')[0] + '_' + str(
                y_str) + '_' + str(y_end) + '_' + str(x_str) + '_' + str(
                    x_end) + '.png'
            lab_patch.save(osp.join(out_dir, 'annotations', mode, str(image)))
...
def main():
    ...
    for split in ['training', 'validation', 'test']:
        mkdir_or_exist(osp.join(out_dir, 'images', split))
    for split in ['training', 'validation']:
        mkdir_or_exist(osp.join(out_dir, 'annotations', split))
    ...
    split_roots = {
        'train': osp.join(dataset_path, 'uavid_train'),
        'val': osp.join(dataset_path, 'uavid_val'),
        'test': osp.join(dataset_path, 'uavid_test')
    }

    available_splits = [k for k, v in split_roots.items() if os.path.exists(v)]
    if not available_splits:
        raise FileNotFoundError(f'No UAVid split folders found in {dataset_path}')

    for split in available_splits:
        dataset_mode = split_map[split]
        img_root = split_roots[split]
        img_paths = glob.glob(os.path.join(img_root, '**', 'Images', '*.png'), recursive=True)
        prog_bar = ProgressBar(len(img_paths))
        ...
        if split != 'test':
            label_paths = glob.glob(os.path.join(img_root, '**', 'Labels', '*.png'), recursive=True)
            lab_bar = ProgressBar(len(label_paths))
            for i, lab_path in enumerate(label_paths):
